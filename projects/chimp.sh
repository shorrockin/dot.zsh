...
ch() {
  ...
  if [[ $1 == 'server' ]]; then
    ...
    rsync -ave ssh deploy@prod:/home/deploy/chimp.net/shared/system/ /Users/chrisshorrock/Work/chimp.net/public/system/
  elif [[ $1 == 'sync-avatars-disco' ]]; then
    rsync -ave ssh /Users/chrisshorrock/Work/chimp.net/public/system/ deploy@disco:/home/deploy/chimp.net/shared/system/
  elif [[ $1 == 'make-db' ]]; then
    mysql -uroot -proot -e "create database $2 character set utf8 collate utf8_general_ci"
  elif [[ $1 == 'browser-stack' ]]; then
    java -jar ~/Work/browserstack.com/BrowserStackTunnel.jar $BROWSER_STACK_KEY chimp.loc,3000,0
  elif [[ $1 == 'routes' ]]; then
    if [[ $# == 2 ]]; then
      rake routes | grep $2
    else
